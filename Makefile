...
## Docker
start: ## Build and start workspaces.
	@docker compose -f ./docker/compose.yaml -p ${PROJECT_NAME} up -d --build

debug-php: ## Build and start the PHP workspace with debugging enabled.
	@XDEBUG_MODE=debug docker compose -f ./docker/compose.yaml -p ${PROJECT_NAME} up -d --build

stop: ## Stop the workspaces.
	@docker compose -p ${PROJECT_NAME} down

enter-php: ## Enter the PHP workspace.
	@docker exec -it ${PROJECT_NAME}-php-1 /bin/bash || true

enter-go: ## Enter the Go workspace.
	@docker exec -it ${PROJECT_NAME}-go-1 /bin/bash || true
